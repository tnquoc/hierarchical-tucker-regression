...
def gen_image(region='swiss', image_height=20, image_width=20, n_channels=None, weight_value=1):
    ...
    if region is "swiss":
    ...
    elif region is "triangle":
        start = image_height // 3 - 1
        end = 2 * image_height // 3
        center = image_width // 2
        for i in range(start, end):
            number_of_point = 2 * (i - start) + 1
            weight[i][center] = 1
            for j in range(center - number_of_point // 2, center + number_of_point // 2 + 1):
                weight[i][j] = 1
        ...
        index = x ** 2 + y ** 2 <= radius ** 2
        weight[cy - radius:cy + radius, cx - radius:cx + radius][index] = 1
        index = x ** 2 + y ** 2 <= (radius - 2) ** 2
        weight[cy - radius:cy + radius, cx - radius:cx + radius][index] = 0
